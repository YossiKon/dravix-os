...
SUB_NUMBER(time)
 ...
 public:
  ...
  // dravix: release the holding torque AFTER the move completes WITHOUT blocking the main
  // loop. The stock driver slept in delay() for the whole travel time (100-1200 ms per head
  // move); a blocked loop drops microphone audio and overflows the speaker. Backends that
  // are Components override this with a scheduled timeout; the default keeps the old
  // blocking behaviour so any other backend is unaffected.
  virtual void release_torque_after(uint32_t ms);
