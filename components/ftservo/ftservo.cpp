...
void FTServo::move_angle(int angle) {
  ...
  if (speed <= 0) speed = 500;  // dravix: never divide by zero below

  this->move(target_pos, time, speed);

  // dravix: was `delay(travel + 100 ms); write_torque_enable(false);` — a blocking sleep in
  // the main loop for every single head move. The release is scheduled instead.
  this->release_torque_after(abs(target_pos - current_pos) * 1000 / speed + 100);
}

// dravix: the default keeps the stock (blocking) behaviour for backends that are not
// Components; SCS9009Servo overrides it with a scheduled timeout.
void FTServo::release_torque_after(uint32_t ms) {
  delay(ms);
