# ftservo (vendored, patched)

Vendored from [m5stack/esphome-yaml](https://github.com/m5stack/esphome-yaml)
`components/ftservo` @ `238ba9825e7874d5626709631dae6be92e861b17` (MIT, see LICENSE — © 2025 m5stack), with one deliberate patch:
...
- `ftservo.h` / `ftservo.cpp`: `move_angle()` schedules the torque release through a new
  virtual `release_torque_after(ms)` instead of sleeping; the default implementation keeps the
  old blocking behaviour for non-Component backends; a speed of 0 can no longer divide by zero.
- `number/ftservo_number.h`: the position number does the same.
