...
BMI270BMM150Sensor::imu_spec_t BMI270BMM150Sensor::getImuRawData(imu_raw_data_t *data)
{
  ...
  uint8_t intstat = 0;
  this->read_register(INT_STATUS_1_ADDR, &intstat, 1);  // dravix: ESPHome 2026.6 i2c API dropped the `stop` arg (stop=true is the behavior)
  ESP_LOGVV(TAG, "intstat: %02X", intstat);
  if (intstat & 0xE0)
  {
    std::int16_t buf[10];
    auto buffer = this->read_register(AUX_X_LSB_ADDR, (std::uint8_t*)&buf, 20);  // dravix: 2026.6 API
...
{
  std::int16_t temp;
  bool res = this->read_register(TEMPERATURE_0_ADDR, (std::uint8_t*)&temp, 2);  // dravix: 2026.6 API
...
bool BMI270BMM150Sensor::read_register_(uint8_t reg, uint8_t data) {
  ...
  this->last_error_ = this->read_register(reg, &data, 1);  // dravix: 2026.6 API
...
bool BMI270BMM150Sensor::write_register_(uint8_t reg, const uint8_t *value, size_t len) {
  ...
  // using defaults for length (1) and stop (true)
  this->last_error_ = this->write_register(reg, value, len);  // dravix: 2026.6 API
  if (this->last_error_ != i2c::ERROR_OK) {
